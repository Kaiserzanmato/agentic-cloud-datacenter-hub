...
export const CountryModal: React.FC<CountryModalProps> = ({ country, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-md animate-fadeIn">
      <div 
        className="relative w-full max-w-2xl bg-slate-900 border border-slate-700/80 rounded-2xl p-6 shadow-2xl space-y-6 text-slate-100 overflow-y-auto max-h-[90vh]"
      ...
      >
        ...
        <div className="flex items-center space-x-4 pr-10">
          ...
          <div>
            <div className="flex items-center space-x-2">
              ...
              <span className="text-xs text-slate-400 font-mono">{country.region}</span>
            </div>
            <h2 className="text-2xl font-bold text-white tracking-tight mt-0.5">{country.name}</h2>
            {country.officialName && (
              <p className="text-xs text-slate-400 font-light">{country.officialName}</p>
            )}
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          <div className="p-3.5 rounded-xl bg-slate-950/60 border border-slate-800">
            <span className="text-[10px] font-mono uppercase text-slate-400 block">Sovereign AI Stance</span>
            <span className="text-xs font-bold text-cyan-400 block mt-1">{country.sovereignAiStatus}</span>
          </div>

          <div className="p-3.5 rounded-xl bg-slate-950/60 border border-slate-800">
            <span className="text-[10px] font-mono uppercase text-slate-400 block">Data Center Capacity</span>
            <span className="text-sm font-extrabold text-white block mt-1">{country.dataCenterCapacityMW} MW</span>
          </div>

          <div className="p-3.5 rounded-xl bg-slate-950/60 border border-slate-800">
            <span className="text-[10px] font-mono uppercase text-slate-400 block">Grid Readiness</span>
            <span className="text-sm font-extrabold text-emerald-400 block mt-1">{country.gridReadinessScore}/100</span>
          </div>

          <div className="p-3.5 rounded-xl bg-slate-950/60 border border-slate-800">
            <span className="text-[10px] font-mono uppercase text-slate-400 block">Active Projects</span>
        ...
        <div className="space-y-4 pt-2">
          ...
          <div className="space-y-3 text-xs text-slate-300">
            <div className="flex items-start space-x-3 p-3 rounded-xl bg-slate-950/40 border border-slate-800/60">
              ...
              <div>
                <span className="font-bold text-white block">Power Grid Stability & Renewable PPAs</span>
            ...
            <div className="flex items-start space-x-3 p-3 rounded-xl bg-slate-950/40 border border-slate-800/60">
              ...
              <div>
                <span className="font-bold text-white block">Sovereign Data Governance & Regulatory Policy</span>
                <p className="text-slate-400 mt-0.5">
        ...
        <div className="flex justify-end pt-2">
          <button
            onClick={onClose}
            className="px-5 py-2 rounded-xl bg-slate-800 hover:bg-slate-700 text-slate-200 text-xs font-medium transition-all cursor-pointer"
          >
            Close Profile
          </button>
        </div>
